...
case "$TASK" in
  ...
  a5)
    MSG="<@&${DATA_ROLE}> [주기 A-5/DATA-05] 추출 인사이트 정합성 게이트 (일 1회, 추출 배치 후 자동 트리거). 입력: insights 신규 추출분(vehicle_spec/pricing/market_trend — GWP-1 Gemini추출 산출). 점검: 추출 타입 스키마 정합·환각·중복 이상치·thresholds. **한 번에 최대 50건까지만 검증**(턴·시간 한도 보호), 초과분은 다음 회차로 미루고 'N건 미검증 대기' 명시. 산출: data/outputs/pending/verdict-extraction-\$(date +%Y%m%d).md (항목별 green/amber/red 분류). **신규 추출분 없으면 조용히 종료(보고 불필요, A-3 패턴).** 완료 후 [APPROVAL:관리팀장] 결재카드. green-all이면 경량 승인 대상. green 인사이트만 다운스트림(content CONT-01) 통과 전제. 참고: GWP-1 라이브 유지 — 이 verdict는 검증·게이트용이며 GWP-1 자체 curate/write와 병행." ;;
  ping)
