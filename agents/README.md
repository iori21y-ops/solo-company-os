...
- 결재는 `_shared/approval-card-policy.md` 기준. **(2026-06-09 개편)** 인터랙티브 버튼 카드 폐지 유지 → **평문 알림 + 모바일 뷰어 링크**, 담당 에이전트가 원 업무 채널/스레드에서 대표님에게 직접 상신.
- 결재 결정은 결재 알림에 **답글 `승인`/`수정 [요청]`/`반려 [사유]`** → 브로커가 파일 이동(pending→approved/rejected) + 원채널 담당 에이전트에 통보 → 이어받아 진행. (브로커 순수 코드 처리, LLM 0)
