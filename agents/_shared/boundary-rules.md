...
<!-- SECTION:START name=pair_7_sales_marketing -->
## 쌍 7(추가): sales ↔ marketing

**경계선**: "상담 신청(리드 생성) 시점"
- 리드 생성 전 = marketing (방문자를 리드로 만드는 것)
- 리드 생성 후 = sales (리드를 체결로 전환하는 것)

**marketing 영역**:
- 블로그·카드뉴스 유입 유도, SEO 키워드 전략
- 랜딩페이지 전환율 최적화 (상담 신청 버튼 앞까지)
- 유입 채널별 성과 분석 (어디서 많이 오나)
- 광고 집행, 시즌 캠페인
...
- CRM 리드 관리, 후속 메시지
- 수수료 ROI 분석
- 리드→체결 전환율 추적 (어디서 온 리드가 잘 전환되나)

**회색지대 판정**:
| 케이스 | 담당 | 이유 |
|--------|------|------|
| 방문자 → 상담 신청 버튼 전 UI·CTA 최적화 | marketing | 리드 생성 전 |
| 상담 신청 데이터 수신 후 처리 | sales | 리드 생성 후 |
| 이탈 방문자 재유입 콘텐츠 | marketing | 리드 미생성 상태 재접촉 |
| 기존 리드 재접촉 (CRM 기반) | sales | 리드 생성 후 |
| 유입 채널별 방문 수·클릭률 | marketing | 채널 성과 |
| 유입 채널별 리드 전환율·체결률 | sales | 리드 품질 분석 |
| UTM·트래킹 파라미터 설계 | marketing | 유입 추적 설계 |
| UTM 기반 리드 귀속 판정 | sales | 리드 귀속 |
| 프로모션·이벤트 콘텐츠 제작 | marketing | 유입 유도 |
| 이벤트 유입 리드 후속 처리 | sales | 리드 생성 후 |

**한 줄 테스트**:
- "상담 신청 버튼을 누르기 전 단계인가?" Yes → marketing
- "상담 신청 데이터가 이미 생성된 후 단계인가?" Yes → sales
<!-- SECTION:END name=pair_7_sales_marketing -->
