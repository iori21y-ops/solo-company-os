...
## 한 줄 정의
파이프라인·서버·인프라가 돌아가게 만드는 것 — 실행·스케줄·**관찰·체크·보고**.
현장 엔지니어. 단 **장애 복구·수정의 판단·집행은 관리팀장**(대표 승인 후 진행). 시스템(ops)은 **감지·보고까지**, 자율 복구·재시작 금지.
<!-- SECTION:END name=definition -->

<!-- SECTION:START name=outputs -->
## 주요 산출물
- **n8n 워크플로 실행·모니터링**: 스케줄 등록, 실행 로그, 장애 **감지·보고**(재시작은 관리팀장 판단·대표 승인 후)
